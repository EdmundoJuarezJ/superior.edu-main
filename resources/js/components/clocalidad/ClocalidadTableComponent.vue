...
export default {
  name: "clocalidad-table-component",
  mounted() {
      ...
      this.$store.dispatch('getClocalidades', { searchQuery: this.searchQuery });
      loading.close();
  },
  computed:{
    ...mapGetters({
  ...
  methods: {
    create() {
      window.location.href = "/add-clocalidad";
    },
    showData(index, row){
        window.location.href = "/show-clocalidad/" + row.id;
    },
    editData(index, row){
        window.location.href = "/edit-clocalidad/" + row.id + "/edit";
    },
    fetchAllData(){
        this.$store.dispatch('getClocalidades');
    },
    deleteData(index, row){
        if(confirm("¿Estás seguro de que quieres eliminar estos datos?")){
            this.$store.dispatch('deleteClocalidad', { id:row.id});
            this.fetchAllData();
        }
    }
  },
